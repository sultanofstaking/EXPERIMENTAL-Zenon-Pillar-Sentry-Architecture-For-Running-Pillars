...
Every time you add a new sentry disable the firewall `sudo ufw disable`, stop the node `sudo systemctl stop go-zenon.service`, add the sentry (step `2` in `./sentrify`) then run `./sentrify` option `4`, `enable ufw`, and `sudo systemctl start go-zenon.service`
